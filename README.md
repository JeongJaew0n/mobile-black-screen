...
- **번인 방지** — 60초마다 글자 위치를 타원 궤도로 미세하게 이동 (AMOLED 잔상 방지).
  끌 수 없다 — 끄면 얻을 것이 없고 잃을 것은 패널 손상이다
- **밝기** — 글자는 흰색 고정, 패널 밝기는 상수 0.30. 표시할 내용이 없으면 0.
  조절 슬라이더는 실측에서 0.30 과 0.00 의 전력 차이가 없어 없앴다.
  시계는 분 경계에 맞춰 분 단위로만 갱신
