...
|  | **완전 검정** (기본) | **오버레이** |
|---|---|---|
| 구현 | 풀스크린 Activity + 몰입 모드 | `TYPE_APPLICATION_OVERLAY` + 포그라운드 서비스 |
| 상태바 / 내비바 | **완전히 숨김** | **둘 다 남음** |
| 다른 앱 UI 렌더링 | 멈춤 | **계속 렌더링** |
| 다른 앱 백그라운드 동작 | 계속 동작 | 계속 동작 |
| 권한 | **없음** | 다른 앱 위에 표시 + 알림 |
| 상시 알림 | 없음 | 있음 (제거 불가) |

두 모드의 장단이 정확히 상충해서 하나로 합칠 수 없다. 그래서 둘 다 제공하고 고르게 한다.
...
> **오버레이 모드의 한계는 우회 불가능하다**
> `TYPE_APPLICATION_OVERLAY` 는 Android 8(O)부터 시스템 UI 위에 그리는 것이
> 의도적으로 금지되어 있다. `FLAG_LAYOUT_NO_LIMITS` 를 포함해 어떤 플래그 조합으로도
> 뚫리지 않는다. 실측(Galaxy S23+ / Android 16)상 창 프레임은 `[0,94][1080,2214]` 로
> 상태바 아래에서 시작해 내비바 위에서 끝나고, z-order 상으로도 두 바가 오버레이보다
> 위에 있다. 기하학·레이어 양쪽에서 막혀 있다.

## 기능

- **무표시 / 시계 / 문장** — 기본값은 무표시. 시계 형식은 24시간·12시간 중 선택
- **번인 방지** — 60초마다 글자 위치를 원 궤도로 미세하게 이동 (AMOLED 잔상 방지)
- **저휘도 텍스트** — 순백 대신 5단계 그레이. 시계는 분 경계에 맞춰 분 단위로만 갱신
