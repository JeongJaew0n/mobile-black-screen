...
- **밝기 손잡이는 하나다.** 글자는 항상 흰색이고, 밝기 조절은 창의 `screenBrightness`
  (0.05~0.80) 하나로만 한다. 보이는 밝기는 프레임버퍼 값 × 패널 밝기라서, 둘 다 깎아 놓으면
  왜 안 보이는지 헤매게 된다 — 실제로 세 번 그랬다.
  `#333333`+패널 0 → 안 보임, `#E6E6E6`+패널 0 → 안 보임, `#E6E6E6`+패널 0.08 → 안 보임.
  **글자색을 다시 회색으로 낮추지 말 것.**
