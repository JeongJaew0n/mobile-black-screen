...
> - Screen Off 방식: **세 모드, 기본은 FULL(접근성 오버레이)**
>   — 쓰던 앱을 그대로 두고 상태바·내비바까지 전부 덮는다
...
| 이슈 | 대응 |
|---|---|
| **AMOLED 번인** — 같은 위치에 시계를 오래 띄우면 잔상이 남음 | 60초마다 **픽셀 시프트**(세로로 긴 타원 궤도). 밝기는 패널 쪽에서 낮춘다 — 글자색을 어둡게 깎으면 판독이 안 된다 |
...
| R6 | **AMOLED 번인** | 중간 | 픽셀 시프트 + 낮은 패널 밝기 |
...
| "모두 다 돌아간다"의 의미 | **해석 A** (다른 앱이 계속 돌아가는 상태에서 화면만 덮기) | 접근성 오버레이(FULL)로 그대로 충족. 중간에 잘못 우회했던 경위는 아래 참조 |
...
**1차 조정 — 하이브리드로.** 해석 A(오버레이)를 선택한 뒤 `TYPE_APPLICATION_OVERLAY` 가
상태바를 덮지 못한다는 사실이 확인되었다. "다른 앱 계속 렌더링"과 "완전한 검정"을 동시에
얻을 수 없다고 판단해, 두 모드를 제공하고 **기본값을 Blackout(앱 전환)** 으로 두었다.

**2차 조정 — 잘못된 전제를 바로잡음.** 위 판단은 틀렸다. "오버레이는 상태바를 못 덮는다"는
`TYPE_APPLICATION_OVERLAY` 에만 해당하는 명제인데 **모든 오버레이로 확대 적용**했다.
접근성 서비스는 초기 분석에서 "최상단 Activity 감지 용도로는 과하다"는 이유로 기각했고,
**오버레이 레이어로서의 가능성은 검토조차 하지 않았다.**

그 결과 사용자가 첫 결정에서 명시적으로 고른 요구("다른 앱들이 계속 돌아가는 상태에서
화면만 덮기")와 정반대인 모드가 기본값이 되었다. `TYPE_ACCESSIBILITY_OVERLAY` 로
FULL 모드를 만들어 바로잡았다(§2 방식 4).
...
**요구사항은 Android에서 실현 가능하며, "화면을 끈다"가 아니라 "화면을 검게 덮는다"로
정의를 바꿔야 정확합니다.** AMOLED 기기에서는 이 둘의 체감 차이가 거의 없습니다.

하드 제약은 하나뿐입니다.

**임의 시점에 화면을 끄는 공개 API가 없습니다.** 그리고 진짜로 끄면 요구사항의
"문장/시간 표시"가 불가능해집니다 — 둘은 모순입니다.

한때 두 번째 하드 제약으로 적었던 "오버레이는 상태바를 덮을 수 없다"는 **창 종류에 따라
갈리는 조건부 제약**이었습니다. `TYPE_ACCESSIBILITY_OVERLAY` 는 상태바·내비게이션 바
위까지 덮으므로, 접근성 권한만 허용하면 **쓰던 앱을 그대로 두고 화면 전체를 덮을 수
있습니다.** 이것이 현재 기본값(FULL)입니다.

iOS는 다른 앱 위에 무언가를 덮거나 백그라운드에서 임의로 실행하는 것이 OS 수준에서
차단되어 있어 원래 요구사항을 그대로 옮길 수 없으므로 **범위에서 제외합니다.**

구체적인 구현 설계·코드·함정 목록은 **[architecture.md](./architecture.md)** 에,
전력 측정 방법과 추정치는 **[power-measurement.md](./power-measurement.md)** 에 있습니다.
