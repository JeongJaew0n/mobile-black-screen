...
**실측값** (Galaxy S23+ / Android 16, 2026-09-07). 설정 라디오 라벨을 `uiautomator dump` 로 읽었다.
예시 시각은 구현 중 13:05 → **09:05** 로 바꿨다 — 한 자리 시각이어야 앞자리 0 이 라벨에서 보인다.

| 로케일 | `HHmm` | `hmm` |
|---|---|---|
| ko | **09:05** (앞자리 0 유지) | 오전 9:05 |
| en-US | 09:05 | 9:05 AM |
| ja | 13:05 (예시 13:05 시점 측정) | 午後1:05 |

여섯 값 모두 CLDR 예상과 일치했고 `MATCH_HOUR_FIELD_LENGTH` 가 살아 있다.
ja 는 문자열 없이 로케일만 바꿔 ICU 결과를 본 것이다 — 라벨 앞부분은 영어 폴백으로 나온다.
...
- **밀어서 잠금 해제 트랙** — 왼→오른쪽으로 미는 물리 제스처다. **여기는 손대야 한다.**
  (핀은 박았다. 다만 사용자가 "아랍어는 필요 없다" 고 해 RTL 실기기 검증은 하지 않았다.
  RTL 언어를 추가하는 날 `ar-XB` 로 확인할 것.)
